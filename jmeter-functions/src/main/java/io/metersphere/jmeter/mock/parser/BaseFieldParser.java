...
import io.metersphere.jmeter.mock.field.*;
...
import io.metersphere.jmeter.mock.util.RegexUtils;
...
public abstract class BaseFieldParser implements FieldParser {
    ...
    protected static String[] getMethods(String name) {
        //获取正则
        String regex = getMethodRegex();
        return RegexUtils.getMatcher(name, regex).toArray(new String[0]);
    }
...
//            System.out.println(methodName);
            ...
            if (method == null) {
                invokerList.add(MethodUtils.createNullMethodInvoker(methodStr));
            } else {
                invokerList.add(MethodUtils.createMethodInvoker(null, params, method));
            }
        }
...
//    private static final Pattern allMethodNameRegex;
    ...
    /**
     * 获取随机整数的方法名，有三种参数：
     * {@link MockUtils#integer()}<em>获取随机数字 0-9</em>
     * {@link MockUtils#integer(Integer)}<em>获取指定长度的随机数,※不可超过int最大上限</em>
     * {@link MockUtils#integer(Integer, Integer)}<em>获取指定区间[a,b]的随机数,※不可超过int最大上限</em>
     */
    ...
    /**
     * 获取随机小数的方法名，有4种参数：
     * {@link MockUtils#doubles(Integer, Integer, Integer, Integer)}<em>获取制定区间[a,b]的小数，指定小数位数[endL,endR]，double类型</em>
     * {@link MockUtils#doubles(Integer, Integer, Integer)}<em>获取制定区间[a,b]的小数，指定小数位数[end]，double类型</em>
     * {@link MockUtils#doubles(Integer, Integer)}<em>获取指定区间[a,b]的小数，默认小数位数为0，double类型</em>
     * {@link MockUtils#doubles(Integer)}<em>获取指定数值为a的小数，默认小数位数为0，double类型</em>
     *
     */
