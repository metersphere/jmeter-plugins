...
import io.metersphere.jmeter.mock.bean.MockObject;
...
class InstructionParser extends BaseFieldParser {
    ...
    public FieldValueGetter parserForArrayFieldValueGetter() {
        ...
        if (match) {
            ...
            //则创建一个空执行者，将参数作为其输出值
            Invoker nullInvoker = MethodUtils.createNullMethodInvoker(instructionStr);
    ...
    public FieldValueGetter parserForNotListOrArrayFieldValueGetter() {
        ...
        if (match) {
            ...
            if(intervalData == null){
                ...
                fieldValueGetter = getObjectFieldValueGetter(MethodUtils.createNullMethodInvoker(instructionStr));
            }else{
                //有区间参数，获取一个对字符串重复输出的Invoker
                fieldValueGetter = getStringFieldValueGetter(new Invoker[]{MethodUtils.createNullMethodInvoker(instructionStr)});
            }
